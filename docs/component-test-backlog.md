...
- [ ] `library` search — debounce, "Load more" cursor exhaustion, and the empty branch are covered by `apps/web/e2e/m7-library-search.spec.ts`. The search lives in the route module `apps/web/src/routes/g.$groupId_.library.tsx` (its `searchError` → "Search failed. Try again." + `refetch()` branch), not a mountable component, so it sits at the route-mount altitude this layer does not yet stand up. Tracked gap: the `searchError` branch is exercised by neither e2e nor a component test; reaching it needs either a route-mount harness or a forced-5xx case added to the e2e spec.
...
- [x] `tracks` dialogs — `create-track-dialog` (RHF + Zod resolver-version tripwire, server-error→name binding, trim/dirty submit gate, reset-on-reopen — the CreateGroupDialog mirror-pair on a distinct file), `track-settings-dialog` (re-hydration on open, the combined form/status/policy dirty gate, the sequential metadata→status→policy save with a metadata failure binding the name field and skipping the later mutations, archived read-only, danger-zone archive confirm success-close vs failure-toast), `add-track-enrollee-dialog` (members query loading/error+refetch/empty fork, candidate filter+sort with the previously-enrolled tag, per-row `pendingUserId` isolation clearing on both success and failure), `leave-track-dialog` (leave-mutation pending gate + latched-error Callout for retry). No audience picker lives in the track dialogs — it exists only in `activity-composer` (already seeded).
