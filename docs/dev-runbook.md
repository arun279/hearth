...
5. Each Part carries an honor-system **Mark complete** control beside its content (reversible). It flips `part_progress.valueJson.completed`; under the `all_parts_complete` completion rule the Activity Record rolls up to complete automatically when the last Part lands, and under `manual_mark` the header surfaces an explicit "Mark activity complete" action. Completion dots and hard-prerequisite locks in the FlowSidebar / PartTabBar come from the same record — a Part whose hard prerequisite is unmet stays navigable but can't be completed.
6. **Reflection** Parts autosave (800 ms debounce) and carry the record-level Visibility selector (Track / Facilitators only / Just me); the override is stored on `activity_records.visibilityOverrideJson`, never per-Part. `minWords` is an advisory meter, never a gate.
7. **Quiz** Parts grade server-side. The answer key (`answerKeyIndex` / `answerKeyRegex` / `explainAfterAnswer`) never reaches participants — `get-activity-for-player` strips it; the player renders the stripped questions and shows per-question feedback (correct / incorrect / no_key, the correct option, the explanation) from the graded `progress.answers`. Submitting (which grades) and the honor-system complete flag are separate actions.
8. `answerKeyRegex` is user-authored, so it is a ReDoS surface. It is screened at compose time by `isAnswerKeyRegexSafe` (`packages/domain/src/parts/quiz-regex-safety.ts`) — a custom static check that rejects an outer-quantified group whose body holds a nested quantifier or a top-level alternation (e.g. `(a+)+$`, `(a|aa)+$`); `safe-regex2` was rejected because it passes `(a|aa)+$` as safe. Grading runs server-side only, re-checks safety, caps short-answer input at 256 chars (`MAX_SHORT_ANSWER_TEXT_LENGTH`), and fails soft to `no_key` on any unsafe or uncompilable pattern. The pattern is never logged. See `docs/tripwires.md` § "Quiz `answerKeyRegex` residual ReDoS risk" for the free-tier 10 ms CPU-cap residual and the RE2 / subrequest follow-up.
9. `Previous` / `Next` chevrons navigate through `flow.displayOrder`. The bottom edge always shows where the participant is and where they can go.
