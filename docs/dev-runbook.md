...
- **e2e** — boots a real `wrangler dev` and applies migrations through the real `wrangler d1 migrations apply` path. This is the authoritative migration-apply gate (see caveat).
...
**Real-wrangler caveat.** `wrangler d1 migrations apply` runs each migration inside one implicit transaction. The in-memory integration harness applies DDL outside a transaction on empty tables, so it can pass a migration the real wrangler path rejects (transaction-wrapping + foreign-key effects). The e2e gate is the faithful check — and for a table-rebuild migration it only reproduces the failure when its local D1 holds rows, so apply such a migration against a **seeded** local D1 before pushing. The rebuild-under-wrangler failure class and its mitigations live in `docs/tripwires.md` § "Database + ORM".
...
3. **Add Parts** from the palette. Six kinds are authorable: Reading, Audio, Video, Reflection, Embed, and Quiz. Reading / Audio / Video each open a Library Item picker filtered to the matching display kind (PDF/doc/image for Reading, audio MIME for Audio, video MIME for Video). Reflection takes a prompt; Embed takes an https URL plus a provider radio (YouTube / Spotify / Generic). Quiz takes one or more questions, each multiple-choice (mark the correct option) or short-answer (a correct answer plus optional accepted alternates, matched case/whitespace/accent-insensitively unless "Match exactly"). Reorder with the up/down arrow buttons; remove with the X. Only attend_session is still hidden — its player surface ships in a later milestone (M13/M14).
...
6. **Quiz** Parts collect per-question answers and grade them on submit (honor-system): each question shows correct / incorrect / ungraded feedback with any author explanation, plus an "N of M graded correct" summary; re-submitting is allowed. Short-answer grading is plain normalized equality — no regex engine: the typed answer is trimmed, whitespace-collapsed, and (unless "Match exactly" is set) lowercased + accent-folded, then tested for membership in the set of `{ correctAnswer } ∪ alsoAccept`. That makes ReDoS structurally impossible (no author-controlled pattern, no backtracking surface) rather than mitigated. Grading runs server-side and answer keys never reach the browser; a question with no correct answer is "ungraded" and excluded from the score denominator.
7. `Mark complete` is an honor-system toggle in the sticky footer. It's always enabled — no quiz-score or `minWords` gate — and clicking flips the active Part to a reversible `Completed` state (`aria-pressed` tracks the state; clicking again un-marks it). Marking advances the header's "N of M Parts complete" count and the completion track beside the "Part X of N" counter; each Part's mark persists across a reload. The control is hidden entirely (not shown disabled) for read-only viewers and after a window closes, so no dead tab stop is left behind. When every Part carries a mark, a "All parts complete" note appears above the footer row — honest scope: this is per-Part marking, not an activity-level completion record (that arrives with Activity Records).
8. `Previous` / `Next` chevrons navigate through `flow.displayOrder` (collapsing to icon-only below `sm`). On the last Part the forward affordance becomes a "Back to track" finish link. The bottom edge always shows where the participant is and where they can go.
