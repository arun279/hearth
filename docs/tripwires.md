...
### Participant content-sharing opt-in — deferred until a content-consuming surface ships

- **Trigger**: a surface that actually READS another participant's record content lands — an anchored discussion thread on a reflection/answer, OR a peer-facing view of reflection prose / quiz answers / part values. Until such a consumer exists, record content stays owner-only (private) and there is no participant-facing content-visibility control at all; the only configurable visibility is the track-level `peerProgressVisibility` (coarse completion facts, never content).
- **Why deferred**: a content opt-in with no surface that consumes shared content would be a toggle that changes nothing observable — a setting ahead of its consumer. The cross-participant content read paths were removed structurally (no code path lets any non-owner read another participant's content), which is the enforcement for "a participant's content is never widened by another actor."
- **Required shape when it returns (dark-pattern-free)**: a facilitator may only ENABLE or DISABLE the opt-in surface for a track — never pre-share content. The content default stays hardcoded private; a facilitator-set "shared" default is FORBIDDEN (a preselected opt-in is a dark pattern and a de-facto override of participant agency). Exposure of any one record's content requires that participant's affirmative, per-record act, every time — never a blanket or sticky "share all my reflections" flag, and never inherited from a track or group default.
- **Location**: `packages/domain/src/track.ts` (`peerProgressVisibility` is the progress-only knob; the content opt-in is NOT it); the future content-read use case + its route + SPA surface.

### Progress roster stays coarse + non-ranked — no social feed

- **Trigger**: a PR proposes adding a leaderboard, ranking, ordering ("who's ahead"), streaks, reactions, or any social-feed affordance to the track progress roster or the per-activity completion chips — anything beyond non-ranked completion facts (`completionState` + `completedAt`) plus, at most, a non-ordered "N of M completed" headline that carries no ranking. Also fires when a PR proposes surfacing enrolled-but-never-started participants ("who hasn't engaged") in either progress surface.
- **Action**: don't ship it. The Participation-Mode design lists "social feed" as an explicit Avoid; the progress surface is deliberately coarse and non-ranked so peer awareness never becomes peer comparison. Retry / part-history counts stay a facilitator-only struggle signal and are never peer-exposed, independent of `peerProgressVisibility`. Any ranking/ordering/streak/reaction reopens the social-feed prohibition and is a design decision to relitigate against the Participation-Mode constraint first — not a routine roster enhancement. The v1 roster deliberately shows only participants who have a record (started at least one activity): a peer-facing "who hasn't started" column is surveillance, not awareness, so it is forbidden by the same constraint. A facilitator-only non-starter view for pacing is a separate, deferred design — it must stay facilitator-scoped, be framed as pacing rather than chasing, and is gated on a deliberate pacing/curation surface that does not exist yet; until then `listTrackProgress` joins no enrollment rows.
- **Location**: `packages/core/src/use-cases/list-track-progress.ts`; `apps/web/src/components/tracks/track-progress-tab.tsx`.
