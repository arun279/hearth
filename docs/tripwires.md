...
- **Trigger**: `ActivityRecord` rows exist as of M10's pulled-forward Activity-Record slice (lazily created on first participant write), so the "once Records exist" precondition holds today. Re-evaluate when M11 lands the durable-Record treatment, or sooner if an orphan-`PartProgress` shape is observed in practice (a Record/progress row pointing at a Part the activity no longer carries).
- **Action**: re-evaluate the four-step orchestration in `packages/core/src/use-cases/update-activity.ts` (body update + `setLibraryRefs` + `setPrerequisites` + `setSuggestedSequences`). Each call is atomic on its own; a mid-sequence failure leaves the body updated but children stale, which the use case docstring concedes. The current "user retries → idempotent wholesale-replace converges" model remains acceptable: `PartProgress` is keyed by stable `partId`, so a mid-sequence failure does not move a learner's credit, and the hazard window is narrow (an `update-activity` failure vs. an in-flight participant write). Either compose the four writes into one D1 batch (requires a port-level rethink — children writes would need to surface from inside the parent UPDATE) or accept the eventual-consistency story explicitly with an integration test pinning the recovery shape.
