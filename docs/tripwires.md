...
- **Location**: `scripts/test-migrate.mjs` + `packages/db/test/migration-seed.sql` (the gate + its seed); `.github/workflows/db-migrate-apply.yml` and `lefthook.yml` (triggers); `packages/db/migrations/` (generated SQL); `packages/db/src/relations.ts` (where `.references()` parents are declared).
