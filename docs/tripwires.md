...
### Quiz `answerKeyRegex` residual ReDoS risk after the v1 guard

- **Status**: the v1 guard shipped with quiz authoring. `isAnswerKeyRegexSafe` (`packages/domain/src/parts/quiz-regex-safety.ts`) is a custom static screen that rejects the dangerous structural shapes (outer-quantified group whose body holds a nested quantifier or top-level alternation) at compose time via `assertQuizAnswerKeysSafe`; grading runs **server-side only** (the key never ships to participants); short-answer input is capped at 256 chars (`MAX_SHORT_ANSWER_TEXT_LENGTH`) as a backtracking backstop; evaluation re-checks safety and fails soft to `no_key`. `safe-regex2` was rejected — it passes `(a|aa)+$` as safe, failing our own accept/reject table. An in-process `Promise.race` timeout was rejected — a synchronous regex monopolizes the single-threaded isolate, so the timer cannot fire until the match already finished (empirically 83 s on `(a+)+$`).
- **Residual**: a regex that evades the static heuristic (false negative) AND is pathological on ≤256-char input can still burn CPU up to the Worker's limit. On the **free tier that cap is 10 ms** — Cloudflare kills the whole grading request (error 1102), not fail-soft, and it counts against the request budget the killswitch defends.
