...
### drizzle-kit table rebuilds fail under `wrangler d1 migrations apply` on FK-parent tables

- **Trigger**: `pnpm db:generate` emits a table-rebuild migration — the `PRAGMA foreign_keys=OFF; CREATE TABLE __new_<t> …; INSERT … SELECT; DROP TABLE <t>; ALTER … RENAME TO <t>; PRAGMA foreign_keys=ON;` shape — for a table that is a foreign-key parent (something `.references()` it). drizzle-kit rebuilds (rather than an in-place `ALTER`) for any change SQLite can't do natively: add/remove a CHECK, UNIQUE, or PRIMARY KEY; change a column type; add/remove NOT NULL; add/drop a foreign key. The only native in-place ALTERs (no rebuild) are RENAME TABLE, RENAME COLUMN, ADD COLUMN, DROP COLUMN.
- **Why it exists**: `wrangler d1 migrations apply` wraps each migration in one implicit transaction, and SQLite treats `PRAGMA foreign_keys` as a no-op inside a transaction — so drizzle's emitted `foreign_keys=OFF` does nothing and FK enforcement stays on. `DROP TABLE <parent>` then implicitly deletes the parent's rows; with populated dependents and `ON DELETE NO ACTION`/RESTRICT that throws `FOREIGN KEY constraint failed`. It fires **only** for a rebuilt FK-parent that has populated dependents — a rebuild of a leaf/orphan table, or any rebuild against empty tables, applies clean. The in-memory integration harness runs DDL outside a transaction on empty tables, so it masks the whole class; only a real `wrangler d1 migrations apply` against populated data reproduces it.
- **Action**: after `pnpm db:generate`, grep the new `.sql` for `__new_` / `DROP TABLE` / `PRAGMA foreign_keys=OFF`; if present on a FK-parent, don't commit as-is. Prefer, in order: (1) **avoid the rebuild** — re-express the change as an in-place ALTER, e.g. `ALTER TABLE x ADD COLUMN c text NOT NULL DEFAULT '…' CONSTRAINT x_c CHECK (c IN (…))` (SQLite supports a named CHECK on `ADD COLUMN`; it validates existing rows in place, no temp table, no DROP), keeping the equivalent named check in the drizzle snapshot so `db:generate` stays a no-op; (2) **split/reorder** so no rebuild-triggering change rides the same step as the one that populates or references the table; (3) only if a rebuild is genuinely unavoidable, hand-shape it FK-safe and verify it under the e2e / real-`wrangler` gate against a **seeded** local D1 — never trust the in-memory harness. The standard schema-change process is in `docs/dev-runbook.md` § 3a.
- **Gate-coverage note** (maintainer decision, due on the next FK-parent reshape — e.g. M13 adding tables/constraints): no pre-merge gate applies migrations against populated, FK-dependent data — DB drift doesn't apply migrations at all, and e2e applies to a fresh/empty persist dir on CI — so this class is only deterministically caught at the post-merge `--remote` apply. Consider a deterministic `check:conventions` rule that fails when a newly-added migration carries the rebuild signature on a FK-parent table; adopt or decline it when that reshape lands.
- **Location**: `packages/db/migrations/` (generated SQL); `packages/db/src/schema/` + `packages/db/src/relations.ts` (where `.references()` parents are declared).
