...
### `drizzle-kit` + `wrangler` FK-parent table-rebuild gate — retire/relax trigger (`db:test-migrate`)

- **Trigger**: either upstream behaviour that the gate compensates for changes. (1) `drizzle-kit` stops emitting the no-op `PRAGMA foreign_keys=OFF; CREATE TABLE __new_<t> …; INSERT … SELECT; DROP TABLE <t>; ALTER … RENAME;` table-rebuild shape — it gains an FK-safe rebuild, or a native in-place path for the changes that force a rebuild today (add/remove CHECK·UNIQUE·PRIMARY KEY, change a column type, add/remove NOT NULL, add/drop a foreign key; the only native ALTERs today are RENAME TABLE·COLUMN, ADD·DROP COLUMN). (2) `wrangler`/D1 changes per-migration transaction control so `PRAGMA foreign_keys=OFF` is honoured inside a migration (statements no longer share one implicit transaction, or FK enforcement can be toggled per migration).
- **Why the gate exists**: `wrangler d1 migrations apply` wraps each migration in one implicit transaction, and SQLite treats `PRAGMA foreign_keys` as a no-op inside a transaction — so drizzle's emitted `foreign_keys=OFF` does nothing and FK enforcement stays on. `DROP TABLE <parent>` then implicitly deletes the parent's rows; with populated dependents under `ON DELETE NO ACTION`/RESTRICT that throws `FOREIGN KEY constraint failed`. It fires **only** for a rebuilt FK-parent with populated dependents — a rebuild against empty tables applies clean, which is why every empty-data gate (DB drift applies nothing; e2e applies to a fresh persist dir) misses it and it surfaced only at the post-merge `--remote` apply (M12). `db:test-migrate` closes that gap deterministically: it seeds a dependent on every foreign-key edge and applies the new migration through the real `wrangler d1 migrations apply` path on pre-push and CI. The remedy when it goes red (re-express as an in-place ALTER, or split so no rebuild rides populated dependents) is named in the gate's own failure output and in `docs/dev-runbook.md` § 3a.
- **Action**: when either trigger fires, re-evaluate whether `db:test-migrate` can be relaxed (drop the populated-apply step) or retired. Until then it stays a required pre-push + CI gate.
- **Location**: `scripts/test-migrate.mjs` + `packages/db/test/migration-seed.sql` (the gate + its seed); `.github/workflows/db-migrate-apply.yml` and `lefthook.yml` (triggers); `packages/db/migrations/` (generated SQL); `packages/db/src/schema/**` and `packages/db/src/auth-tables.ts` (where `.references()` parents are declared).
