...
- **Why staged this way**: the five signals (`word_count` + `draft_saved_at` for reflection; `answers_submitted` + `last_answered_at` + `auto_score` for quiz) cannot be persisted safely until M17's batcher + limiter exist — a live write per autosave/submit would breach the write budget that backs the $0 guarantee. M11 shipped the port surface (`flushEvidenceSignals` on `ActivityRecordRepository`) and the enqueue calls at both use cases, with the adapter implementing `flushEvidenceSignals` as a gate-branded no-op (an integration test asserts zero `evidence_signals` rows are written). The signal _values_ are already computed in both use cases.
