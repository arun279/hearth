...
### Revision-bump fan-out is synchronous — bounded by the 50-subrequest ceiling

- **Trigger**: a revision bump on a heavily-used Library Item exceeds the free-plan 50-subrequests-per-request ceiling (`docs/free-tier-guardrails.md`, Workers Subrequests row). `finalizeLibraryUpload` reopens every unpinned Library-backed Part across every activity using the item, synchronously inside the finalize request: each affected record costs ~3 D1 subrequests (`reopenAgainstRevision`'s reads + batch) on top of the finalize baseline and the per-activity `byId` + paged `listByActivity`. ~12 participant records on one activity already approaches the ceiling, and a 12–15-person track is a realistic v1 size — so this is in reach sooner than the "bounded for v1" inline comment implies.
- **Action**: drain the fan-out off the request path via a Scheduler queue / cron consumer. `revisionBumpRestart` is idempotent per `(record, newRevisionId)`, so moving it off-request does not change semantics — the trigger that adds the revision can enqueue the affected activity ids and a consumer drains them.
- **Partial-failure shape to fix in the same pass**: the fan-out runs after `addRevision` but before `deletePending`. If finalize throws mid-fan-out, the revision is already added, the fan-out is partially applied, and `deletePending(uploadId)` never runs — leaving an orphan pending-upload row. A retried finalize re-runs the idempotent fan-out cleanly, but the orphan pending row needs the pending-uploads sweep (or an explicit re-finalize) to clear. The off-request consumer should make the revision-add + pending-delete the atomic unit and treat the fan-out as separately retryable.
- **Location**: `packages/core/src/use-cases/finalize-library-upload.ts` (the synchronous `for` over `usingActivities`); `packages/core/src/use-cases/revision-bump-restart.ts` (the idempotent restart primitive).

### `part_history` recency reads are not index-backed

- **Trigger**: per-Part history reads (`listPartHistory`, `reopenAgainstRevision`) become a latency or rows-read hotspot as records accumulate many history rows. They `ORDER BY recorded_at DESC`, but the only `part_history` index is `part_history_record_part_idx` on `(activity_record_id, part_id)` — no `recorded_at` column — so the sort is not index-backed. `countPartHistory` and `partsWithHistory` (the `SELECT DISTINCT part_id` projection) DO ride that index. Correct and cheap at v1 history depths; the M0 schema is frozen, so no migration now.
- **Action**: when a perf pass is warranted, add a covering index `(activity_record_id, part_id, recorded_at DESC)` in a focused migration and re-confirm the recency reads plan against it. Until then the unindexed sort is acceptable at v1 scale.
- **Location**: `packages/db/src/schema/records.ts` (`partHistory`, `part_history_record_part_idx`); `packages/adapters/cloudflare/src/activity-record-repository.ts` (`listPartHistory`, `reopenAgainstRevision` `orderBy(desc(recordedAt))`).
