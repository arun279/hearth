...
- **Viewability before authorization on mutation routes.** A mutation use case for a hideable resource (anything with `canViewGroup`-shaped visibility) MUST run the viewability check first and surface its denial as `DomainError("NOT_FOUND", …)`, then run the role/permission check. The shape: `viewability → 404` (hides existence), then `authorization → 403` (acknowledges existence to authorized viewers). Skipping the viewability check leaks group/track/etc. existence to non-members via the 403-vs-404 status-code distinction — the same enumeration oracle pattern flagged on PR #8 must not recur. Canonical example: `packages/core/src/use-cases/archive-group.ts`.
...
| Skeleton stubs throwing `"Not implemented"` in repository adapters                                                                                                                                                                                                                                            | `packages/adapters/cloudflare/src/*-repository.ts` (learning-track, library-item, learning-activity, activity-record, study-session; plus `user.deleteIdentity`, R2 `getDownloadUrl`) | the first use case calling that method                      |
