...
describe("problem.ts envelope mapping", () => {
  ...
  it("uses the empty-message fallback for unknownErrorProblem", () => {
    const empty = unknownErrorProblem(new Error(""));
    expect(empty.detail).toBe("Unexpected server error.");
    const nonError = unknownErrorProblem("bare-string");
    expect(nonError.detail).toBe("Unexpected server error.");
  });
