...
import { canEnrollInTrack } from "../src/policy/can-enroll-in-track.ts";
...
describe("canEnrollInTrack", () => {
  it("allows a current member on an active track", () => {
    expect(canEnrollInTrack(actor, activeTrack, membership).ok).toBe(true);
  });
  it("denies on an archived track", () => {
    const r = canEnrollInTrack(actor, { ...activeTrack, status: "archived" }, membership);
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.reason.code).toBe("track_archived");
  });
  it("denies when membership is missing", () => {
    const r = canEnrollInTrack(actor, activeTrack, null);
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.reason.code).toBe("not_a_member");
  });
  it("denies when membership is removed", () => {
    const r = canEnrollInTrack(actor, activeTrack, { ...membership, removedAt: now });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.reason.code).toBe("not_a_member");
  });
});
